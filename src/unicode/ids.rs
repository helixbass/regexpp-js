...
static LARGE_ID_START_RANGES: Lazy<Vec<CodePoint>> = Lazy::new(init_large_id_start_ranges);

static LARGE_ID_CONTINUE_RANGES: Lazy<Vec<CodePoint>> = Lazy::new(init_large_id_continue_ranges);
...
fn init_large_id_start_ranges() -> Vec<CodePoint> {
    restore_ranges(
...
fn init_large_id_continue_ranges() -> Vec<CodePoint> {
    restore_ranges(
...
fn is_in_range(cp: CodePoint, ranges: &[CodePoint]) -> bool {
    let mut l = 0;
